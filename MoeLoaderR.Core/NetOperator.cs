...
public class NetOperator
{
    ...
    public ReceiveProgressHandler ReceiveProgressHandler { get; set; }
    ...
    public NetOperator()
    {
        ...
        ReceiveProgressHandler = new ReceiveProgressHandler(HttpClientHandler);
        Client = new HttpClient(ReceiveProgressHandler);
    }

    public void InitReceiveProgressHandler()
    {
        switch (GetProxyMode(Settings, Site.SiteSettings))
        {
            ...
            case Settings.ProxyModeEnum.Custom:
            {
                ...
                try
                {
                    switch (Settings.ProxyConnectMode)
                    {
                        ...
                        case Settings.ProxyConnectModeEnum.Socks:
                            ReceiveProgressHandler = new ReceiveProgressHandler(new SocketsHttpHandler
                            {
                ...
                {
                    Ex.Log(e);
                    ReceiveProgressHandler = new ReceiveProgressHandler(new HttpClientHandler());
                }
                break;
            }
            case Settings.ProxyModeEnum.Ie:
                ReceiveProgressHandler = new ReceiveProgressHandler(new HttpClientHandler { Proxy = WebRequest.DefaultWebProxy });
